...
void Allocator::mergeBoth(BlockHeader* previous, BlockHeader* current, BlockHeader*
	next) {
...
void* Allocator::expandRight(void* addr, size_t size) {
	...
	int deltaSize = (int)(current -> size) - (int)(size); //TODO: fis expression to int-fixed
		...
		//set BlockHeaders
			...
			initBlockHeader(next, false, current -> size, area - size -

				sizeof(BlockHeader) / sizeof(int), 7);
...
void* Allocator::expandBoth(void* addr, size_t size) {
	...
	BlockHeader* previous = previousBlockHeader(current);
	BlockHeader* next = nextBlockHeader(current);
	size_t area = current -> size + previous -> size + next -> size +
		2 * sizeof(BlockHeader) / sizeof(int);
...
void Allocator::initBlockHeader(BlockHeader* bh, bool state, size_t previous, size_t
	size, int mask) {
	if (bh == NULL)
		return;
	if (mask&4) {
...
void Allocator::mem_dump() {
	...
	while (current != NULL) {
		cout& << i <<  ". " << current& << "  " << current -> state << "   " << 

		current -> size << "   " << current -> prevsize& << endl;
