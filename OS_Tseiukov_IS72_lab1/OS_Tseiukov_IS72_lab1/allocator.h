...
class Allocator {
public:
	Allocator(const int n);		//return addr on begin of allocated block or NULL
...
private:
	...
	void mergeWithPrevious(BlockHeader* previous, BlockHeader* current, BlockHeader*
		next);
	...
	//next founctions merde 2 or 3 blocks (one is use), then separate them on use and
	freeand copy data to new use
	...
	void initBlockHeader(BlockHeader* bh, bool state, size_t previous, size_t size,
		int mask);//mask 7 (binary: 111) - all
